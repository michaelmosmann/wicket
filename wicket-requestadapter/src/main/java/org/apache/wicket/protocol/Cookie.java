...
public class Cookie implements Cloneable
{
	 ...
	 * characters and cannot contain commas, semicolons, or white space or begin with a $ character.
	 * The cookie's name cannot be changed after creation.
	 * 
	 * <p>
	 * The value can be anything the server chooses to send. Its value is probably of interest only
	 * to the server. The cookie's value can be changed after creation with the
	 * <code>setValue</code> method.
	 * 
	 * <p>
	 * By default, cookies are created according to the Netscape cookie specification. The version
	 * can be changed with the <code>setVersion</code> method.
	 * 
	 * 
	 * @param name
	 ...
	 *             if the cookie name contains illegal characters (for example, a comma, space, or
	...
	public Cookie(String name, String value)
	{
		...
		{
			throw new IllegalArgumentException(MessageFormat.format("cookie name is token: {}",
				name));
		}
	 ...
	 * presents the cookie to the user. Comments are not supported by Netscape Version 0 cookies.
	 * 
	 * @param purpose
	 ...
	 * <code>.foo.com</code>) and means that the cookie is visible to servers in a specified Domain
	 * Name System (DNS) zone (for example, <code>www.foo.com</code>, but not
	 * <code>a.b.foo.com</code>). By default, cookies are only returned to the server that sent
	 * them.
	 * 
	 * 
	 * @param pattern
	 *            a <code>String</code> containing the domain name within which this cookie is
	 ...
	 * 
	 * <p>
	 * The cookie is visible to all the pages in the directory you specify, and all the pages in
	 * that directory's subdirectories. A cookie's path must include the servlet that set the
	 * cookie, for example, <i>/catalog</i>, which makes the cookie visible to all directories on
	 * the server under <i>/catalog</i>.
	 * 
	 * <p>
	 * Consult RFC 2109 (available on the Internet) for more information on setting path names for
	 * cookies.
	 * 
	 * 
	 * @param uri
	 ...
	 * 
	 * @param flag
	 *            if <code>true</code>, sends the cookie from the browser to the server only when
	 *            using a secure protocol; if <code>false</code>, sent on any protocol
	 * 
	 * @see #getSecure
	 * 
	 */
	 ...
	 * 
	 * @return <code>true</code> if the browser uses a secure protocol; otherwise, <code>true</code>
	 * 
	 * @see #setSecure
	 * 
	 */
	...
	/**
	 * 
	 * Assigns a new value to a cookie after the cookie is created. If you use a binary value, you
	 * may want to use BASE64 encoding.
	 * 
	 * <p>
	 ...
	 * the original Netscape cookie specification. Version 1 complies with RFC 2109.
	 * 
	 * <p>
